...
fn proper_instantiation(){
    ...
    // decode 64 a public key
    let pubkey_bytes = base64::decode("A9EkWupSnnFmIIEWG7WtMc0Af/9oEuEeSRTKF/bJrCfh");

    // let pubk = pubkey_bytes.as_slice();

    println!("pubk: {:?}", pubkey_bytes);

    assert_eq!("Aura 49731", value.name);
